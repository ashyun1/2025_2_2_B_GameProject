...
public class DeliveryOrderSystem : MonoBehaviour
{
    ...
    [Header("주문 설정")]
    public float ordergenrateInterval = 15f;  //주문 생성시간
    ...
    public int GetPickWaitingCount()
    {
        ...
        {
            if (order.state == OrderState.WatingPickup) count++;
        }
    ...
    DeliveryOrder FindOrderForPickup(Building restaurant)
    {
        foreach (DeliveryOrder order in currentOrders)
        {
            if (order.restaurantBuilding == restaurant && order.state == OrderState.WatingPickup)
            {
                return order;
    ...
    public void OnDriverEnteredRestaurnat(Building restaurant)
    {
    ...
    private void OnGUI()
    {
        ...
        foreach(DeliveryOrder order in currentOrders)
        {
            string status = order.state == OrderState.WatingPickup ? "픽업 대기" : "배달대기";
